...
func Day3(input string) (int, int) {
	...
	inProg := 0
	startX := 0
	symbols := map[int]map[int]rune{
		0: {},
	}
	stars := map[int]map[int][]int{
		0: {},
	}
	type num struct {
		startX int
	...
	for i := 0; i < len(input); i++ {
		...
		if c < '0' || c > '9' {
			if inProg != 0 {
				nums = append(nums, num{startX, x - 1, y, inProg})
				//fmt.Println(startX, x-1, y, inProg)
				inProg = 0
			}
		}
		if c == '\n' {
		...
		symbols[y][x] = rune(c)
		//fmt.Println(x, y, string(c))
		x++
	}

	total := func(n num) int {
		for y := n.y - 1; y <= n.y+1; y++ {
			for x := n.startX - 1; x <= n.endX+1; x++ {
				if _, ok := stars[y][x]; ok {
					stars[y][x] = append(stars[y][x], n.num)
				}
				if symbols[y][x] != 0 {
					return n.num
				}
...
func TestDay3_Ex1(t *testing.T) {
	a, b := Day3(exD3)
	assert.Equal(t, a, 4361)
	assert.Equal(t, b, 467835)
}

func TestDay3_Actual(t *testing.T) {
	a, b := Day3(inputD3)
...
const inputD3 = `..224.....487...................718.....................378............................................284........310......313..........311.
